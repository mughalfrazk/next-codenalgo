...
export const emailStatuses = ['pending', 'sent', 'skipped', 'failed'] as const
export type EmailStatus = (typeof emailStatuses)[number]

/** A persisted enquiry — the input plus delivery/record metadata. */
export const contactSubmissionSchema = contactSubmissionInputSchema.extend({
  id: z.string().min(1),
  createdAt: z.number(),
